...
# Import raw data
```{r}
symbiont <- read.csv("merged_hybridref_counts_Ppor_Sym.csv", header=TRUE, row.names=1)
dim(symbiont)  
...
symbiont_means_filtered <- symbiont_means_filtered[, 1:10]
...
p2 <- ggplot(symbiont_means_filtered) +
  geom_histogram(aes(x = R_052_La_Pp_yr2), stat = "bin", bins = 200) +
  xlim(-5, 500)  +
  xlab("Raw expression counts") +
  ylab("Number of genes")
...
saveRDS(vsdBlindFalse, file = "counts_vst.BlindFalse_Ppor_symbiont.rds")

# later, after you already created the file, can load the existing file:
#vsdBlindFalse <- readRDS("counts_vst.BlindFalse_Ppor_symbiont.rds")
```
...
saveRDS(normalized_counts, file = "counts_normalized_Ppor_symbiont.rds")
...
write.csv(normalized_counts, "counts_normalized_Ppor_symbiont.csv", row.names = F)
```
...
# save rlogged file
saveRDS(DE_RO_RC_pvalue, file = "Ppor_symbiont_RO_RC_pval.05.rds")
write.csv(DE_RO_RC_pvalue, "Ppor_symbiont_RO_RC_pval.05.csv")
```
...
```{r}
mcols(results_RO_RC_.1)$description
```
