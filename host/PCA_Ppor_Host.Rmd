...
```{r}
norm.genes <- readRDS(file = "counts_normalized_Ppor_Host.rds")
...
```{r}
de.genes.RO_RC <- readRDS(file = "Ppor_Host_RO_RC_pval.05.rds")
...
```{r}
# save plot
#pdf("Ppor_Host_PCA_total-exp_group.pdf")
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Reef-Ojo vs. Reef-Control (196 genes)") +
      labs(color = "Origin.Destination", fill = "Origin.Destination") +
      theme_cowplot()
...
```{r}
# save plot
pdf("Ppor_Host_PCA_RO_RC_all-indiv.pdf")
...
```{r}
pairwise.adonis2(dist_overall ~ group, data = meta, perm = 9999)
```
