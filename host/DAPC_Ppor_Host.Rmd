...
```{r}
counts.transformed.vst <- readRDS(file = "counts_vst.BlindFalse_Ppor_Host.rds")
```
...
From here, we can narrow the search by specifying the number of PC to try with n.pca and centering it around 3, and doing 100(0) replicates each.

***NB! This will take a long time ! (for 100, 1000 replicates)***
```{r}
xval2 <- xvalDapc(dat, meta$group, n.pca.max = 300, n.da = NULL, training.set = 0.9, result = "groupMean", center = TRUE, scale = FALSE, n.pca = 1:5, n.rep = 100, xval.plot = TRUE)
```
...
- var (proportion of conserved variance): 0.786
...
```{r}
dp2 = dapc(dat, meta$group, n.pca = 3, n.da = 1)
dp2
```

## scatter plot

*Only used 1 discriminant function so cannot make the scatter plot as per other species*
```{r}
...
```{r}
# save plot
pdf("Ppor_Host_DAPC_scatter.pdf")
...
```{r}
# save plot
pdf("Ppor_Host_DAPC_density.pdf")
